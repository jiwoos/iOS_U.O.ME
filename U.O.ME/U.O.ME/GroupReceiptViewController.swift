...
//  U.O.ME
//
//  Created by Anderson Gonzalez on 7/28/20.
...
class GroupReceiptViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBOutlet weak var submitButton: UIButton!
      ...
      func addToDatabase(_ date: String, _ merchant: String, _ subtotal: Double, _ tax: Double, _ total: Double){
        ...
        var payed = [String:String]()
        for debtor in self.theGroupMebers{
                payed[debtor] = "false"
        }

           
        //created a new item in receipts branch
        let  newReceiptRef = self.ref.child("groupData/\(self.theGroupId)/receipts").childByAutoId()
        let  newReceiptKey = newReceiptRef.key
        
        
        //the components of the new item
        var receiptVal = ["Date": date, "Merchant": merchant, "Subtotal":subtotal, "Tax":tax, "Total" : total, "imageURL" : "", "processed": "false",  "deleted":"false", "payed":payed, "payedBy" : self.theUserName, "priceSplit" : total/Double(self.theGroupMebers.count+1)] as [String : Any]
        

        //add the componets to new item in database
        newReceiptRef.setValue(receiptVal, withCompletionBlock: { error, ref in
            if error != nil{
                print("ERROR")
            }
        })
        
        //upload image to storage and then update image url
        guard let imageData = self.imageSelected?.jpegData(compressionQuality: 0.5) else { return }
                   ...
                   let storageProfileRef = storageRef.child("/groupReceipt/").child(self.theGroupId).child(newReceiptKey!)
                        ...
                        { (StorageMetadata, error ) in
                            ...
                            storageProfileRef.downloadURL(completion: { (url, error) in
                                if let metaImageURL = url?.absoluteString {
                                    receiptVal["imageURL"] = metaImageURL
                                    print(metaImageURL)
                    
                                       newReceiptRef.updateChildValues(receiptVal, withCompletionBlock: { error, ref in
                                        if error != nil{
                                            print("ERROR")
                                        }
                                    })
